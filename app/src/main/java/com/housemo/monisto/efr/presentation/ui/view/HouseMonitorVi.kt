...
class HouseMonitorVi(
    private val houseMonitorContext: Context,
    private val houseMonitorCallback: HouseMonitorCallBack,
    private val houseMonitorWindow: Window
) : WebView(houseMonitorContext) {
    private var houseMonitorFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun houseMonitorSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.houseMonitorFileChooserHandler = handler
    }
    init {
        ...
        webSettings.apply {
            ...
            userAgentString = WebSettings.getDefaultUserAgent(houseMonitorContext).replace("; wv)", "").replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        houseMonitorContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(houseMonitorContext, "This application not found", Toast.LENGTH_SHORT).show()
                    }
                    true
                }
            }
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("https://test-web.syndi-test.net") != true) {
                    HouseMonitorApplication.houseMonitorInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(HouseMonitorApplication.HOUSE_MONITOR_MAIN_TAG, "onPageFinished : ${HouseMonitorApplication.houseMonitorInputMode}")
                    houseMonitorWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    houseMonitorEnableStableInputScroll()
                } else {
                    HouseMonitorApplication.houseMonitorInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE
                    Log.d(HouseMonitorApplication.HOUSE_MONITOR_MAIN_TAG, "onPageFinished : ${HouseMonitorApplication.houseMonitorInputMode}")
                    houseMonitorWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                houseMonitorFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            override fun onCreateWindow(
                ...
                houseMonitorHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    fun houseMonitorFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun houseMonitorEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    private fun houseMonitorHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView = HouseMonitorVi(houseMonitorContext, houseMonitorCallback, houseMonitorWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            houseMonitorCallback.houseMonitorHandleCreateWebWindowRequest(windowWebView)
        }
    }
